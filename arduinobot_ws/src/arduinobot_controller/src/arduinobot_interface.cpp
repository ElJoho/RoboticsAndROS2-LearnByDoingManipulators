...
namespace arduinobot_controller{
        ...
        if(arduino_.IsOpen()){
            try{
                ...
                RCLCPP_FATAL_STREAM(
                    ...
                    "Something went wrong while closing the connection wth port " << port_
                );
            }
    ...
    CallbackReturn ArduinobotInterface::on_init(const hardware_interface::HardwareInfo &hardware_info){
        ...
        try{
            ...
            RCLCPP_FATAL(rclcpp::get_logger("ArinobotInterface"),"No serial port provided! Aborting");
    ...
    CallbackReturn ArduinobotInterface::on_activate(const rclcpp_lifecycle::State & previous_state){
        ...
        try{
            ...
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while interacting with the port " << port_);
            return CallbackReturn::FAILURE;
        }
    ...
    CallbackReturn ArduinobotInterface::on_deactivate(const rclcpp_lifecycle::State & previous_state){
        ...
        RCLCPP_INFO(rclcpp::get_logger("ArduinobotInterface"),"Hardware stopped, ready to take commands ...");
        return CallbackReturn::SUCCESS;
    }
    ...
    hardware_interface::return_type  ArduinobotInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period){
        ...
        try{
            ...
            RCLCPP_ERROR_STREAM(
                rclcpp::get_logger("ArduinobotInterface"),
                "Something went wrong while sending the message " << msg << " to the port "  << port_
            );
